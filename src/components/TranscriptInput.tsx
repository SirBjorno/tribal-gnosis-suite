    ...
    <li className="p-4 bg-white rounded-lg shadow-sm border border-slate-200/80 hover:shadow-md hover:border-sky-300 transition-all duration-200">
      <div className="flex justify-between items-center">
        <div>
          ...
          <div
            className={`inline-flex items-center gap-1.5 mt-1 text-xs font-medium px-2 py-0.5 rounded-full ${config.color}`}
          >
...
const ReviewQueue: React.FC<{
  ...
  items: ReviewItem[];
  onSelectItem: (item: ReviewItem) => void;
}> = ({ title, items, onSelectItem }) => (
  <div className="bg-slate-50/70 p-4 rounded-xl border border-slate-200/80 flex-1 min-w-[300px]">
    <div className="flex items-center gap-3 mb-4 px-2">
      <QueueIcon />
      <h2 className="text-xl font-bold text-slate-800">{title}</h2>
      <span className="text-sm font-bold bg-slate-200 text-slate-600 w-6 h-6 flex items-center justify-center rounded-full">
        {items.length}
      </span>
    </div>
    {items.length > 0 ? (
      <ul className="space-y-3">
        {items.map((item) => (
          <Ticket key={item.id} item={item} onSelect={() => onSelectItem(item)} />
        ))}
      </ul>
    ) : (
...
const TranscriptInput: React.FC<TranscriptInputProps> = ({ items, onSelectItem, isProcessing }) => {
  const transcriptsForReview = items.filter(
    (i) => i.status === 'pending_transcript' || (i.status === 'summarizing' && isProcessing)
  );
  const summariesForApproval = items.filter((i) => i.status === 'pending_summary');

  return (
    <div
      className={`bg-white p-6 rounded-xl shadow-lg border border-slate-200/80 mb-8 transition-opacity duration-300`}
    >
      <div className="flex flex-col lg:flex-row gap-6">
        <ReviewQueue
          title="Transcripts for Review"
          items={transcriptsForReview}
          onSelectItem={onSelectItem}
        />
        <ReviewQueue
          title="Summaries for Approval"
          items={summariesForApproval}
          onSelectItem={onSelectItem}
        />
      </div>
    </div>
