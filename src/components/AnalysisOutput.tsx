...
const EditableSummaryField: React.FC<{
  label: string;
  value: string;
  onChange: (value: string) => void;
  icon: React.ReactNode;
  isTextarea?: boolean;
}> = ({ label, value, onChange, icon, isTextarea = false }) => (
  <div className="flex items-start gap-3">
    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 mt-1">
      {icon}
    </div>
  ...
  return (
    <div className="space-y-4">
      <EditableSummaryField
        label="Customer Name"
        value={summary.customerProfile.name}
        onChange={(val) => handleChange('name', val)}
        icon={<CustomerProfileIcon />}
      />
      <EditableSummaryField
        label="Account Number"
        value={summary.customerProfile.accountNumber}
        onChange={(val) => handleChange('accountNumber', val)}
        icon={<CustomerProfileIcon />}
      />
      <EditableSummaryField
        label="Product Information"
        value={summary.productInformation}
        onChange={(val) => handleChange('productInformation', val)}
        icon={<LightbulbIcon />}
      />
      <EditableSummaryField
        label="Problem"
...
const EditableDialogueLine: React.FC<{
  entry: DialogueEntry;
  ...
  return (
    <div
      className={`flex gap-3 text-sm p-2.5 rounded-md ${isAgent ? 'bg-sky-50/70' : 'bg-slate-50/70'}`}
    >
      <div className="font-mono text-slate-400 text-xs pt-2.5">{entry.timestamp}</div>
      <div className="flex-1">
        <p className={`font-semibold ${isAgent ? 'text-sky-800' : 'text-slate-800'}`}>
          {entry.speaker}
        </p>
        <textarea
          value={entry.text}
...
const TranscriptEditor: React.FC<{
  ...
  onTranscriptChange: (newTranscript: DetailedTranscript) => void;
  audioUrl: string | null;
}> = ({ transcript, onTranscriptChange, audioUrl }) => {
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        <p className="text-sm font-medium text-slate-600">
          ...
          <span className={`font-bold ${confidenceColor}`}>
            {(transcript.confidenceScore * 100).toFixed(0)}%
          </span>
        </p>
      </div>
      <div className="bg-white p-3 border border-slate-200/80 rounded-md max-h-[50vh] overflow-y-auto space-y-2">
        {transcript.dialogue.map((entry, index) => (
          <EditableDialogueLine
            key={index}
            entry={entry}
            onTextChange={(newText) => handleDialogueChange(index, newText)}
          />
        ))}
      </div>
    </div>
...
const AnalysisOutput: React.FC<AnalysisOutputProps> = ({
  ...
  const [editedTranscript, setEditedTranscript] = useState<DetailedTranscript>(() =>
    JSON.parse(JSON.stringify(item.detailedTranscript))
  );
  const [editedSummary, setEditedSummary] = useState<Analysis | null>(() =>
    item.summary ? JSON.parse(JSON.stringify(item.summary)) : null
  );
          ...
          <div className="text-sm">
            {isApproveDisabled && (
              <p className="text-amber-700">
                Internet connection is required to approve & summarize.
              </p>
            )}
          </div>
