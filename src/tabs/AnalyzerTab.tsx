...
import { analyzeTranscript, searchSessionSolutions, generateDetailedTranscript } from '../services/apiService';

import DataManager from '../components/DataManager';
import ErrorMessage from '../components/ErrorMessage';
import Loader from '../components/Loader';
import SearchResults from '../components/SearchResults';
import { UploadIcon, SearchIcon, CustomerProfileIcon, ProblemIcon, LightbulbIcon, MicrophoneIcon } from '../components/Icons';
...
const AnalysisDisplay: React.FC<{ analysis: Analysis }> = ({ analysis }) => (
  <div className="space-y-6">
    <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-sky-100 flex items-center justify-center text-sky-600 mt-1"><CustomerProfileIcon className="w-6 h-6"/></div>
        <div>
    ...
    <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-red-600 mt-1"><ProblemIcon className="w-6 h-6"/></div>
        <div>
    ...
    <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-600 mt-1"><LightbulbIcon className="w-6 h-6"/></div>
        <div>
            <h3 className="text-lg font-semibold text-slate-800">Solution & Resolution</h3>
            <p className="text-slate-600"><span className="font-semibold">Action Taken:</span> {analysis.solution}</p>
            <p className="text-slate-600"><span className="font-semibold">Outcome:</span> {analysis.resolution}</p>
        </div>
    </div>
  </div>
);
...
const AudioProcessor: React.FC<{ onProcess: (file: File) => void; isTranscribing: boolean; isOnline: boolean; }> = ({ onProcess, isTranscribing, isOnline }) => {
            ...
            >
                ...
                {isTranscribing ? (
                    ...
                    <div className="text-center py-6 pointer-events-none">
                        ...
                        <p className="mt-2 text-sm font-semibold text-slate-700">Drag & drop or click to upload</p>
                        <p className="text-xs text-slate-500">(MP3, WAV, etc)</p>
                        {!isOnline && <p className="text-xs text-amber-600 mt-2">Internet connection required for processing.</p>}
                    </div>
                )}
            </div>
        </div>
...
const AnalyzerTab: React.FC<AnalyzerTabProps> = ({ setReviewItems, setActiveTab, tenantId, liveCall, setLiveCall }) => {
  ...
  const handleAudioProcess = (file: File) => {
    ...
    generateDetailedTranscript(simulatedRawTranscript)
        .then(detailedTranscript => {
            ...
            setReviewItems(prev => prev.map(item => item.id === placeholderId ? finalReviewItem : item));
        })
        .catch(err => {
            ...
            const errorItem: ReviewItem = {
                ...
                detailedTranscript: { 
                    ...
                    dialogue: [{ speaker: 'Unknown', timestamp: '00:00', text: `Failed to process ${file.name}. Please try again.` }] 
                },
            };
            setReviewItems(prev => prev.map(item => item.id === placeholderId ? errorItem : item));
        });
  };
  ...
  const handleExport = () => {
    const blob = new Blob([JSON.stringify(sessionKnowledgeBank, null, 2)], { type: 'application/json' });
  ...
  const handleImport = (file: File) => {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (Array.isArray(content) && content.every(item => 'problem' in item && 'solution' in item)) {
  ...
  return (
    <div className="space-y-8">
      ...
      {!liveCall && (
        <>
            <AudioProcessor onProcess={handleAudioProcess} isTranscribing={isTranscribing} isOnline={isOnline} />
            <hr className="border-slate-200/80" />
            <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-900">Private Session Analyzer</h2>
                <p className="text-slate-500">Use this tool for temporary, in-session analysis that is not sent to the workflow.</p>
            </div>
        </>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
            <RealTimeTranscriber
                ...
                liveCall={liveCall}
            />
            {!liveCall && <DataManager onImport={handleImport} onExport={handleExport} isOnline={isOnline} transcriptCount={sessionKnowledgeBank.length} />}
        </div>
      ...
      {!liveCall && (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200/80">
            ...
            <p className="text-sm text-slate-500 mb-4">
                Find solutions from the <span className="font-bold">{sessionKnowledgeBank.length}</span> transcript{sessionKnowledgeBank.length !== 1 && 's'} analyzed in this session.
            </p>
            ...
            <button 
                    ...
                    title={!isOnline ? "Cannot search: internet is offline" : (sessionKnowledgeBank.length === 0 ? "Analyze a transcript to enable search" : "Search for solutions")}
            ...
            {!isSearching && searchResults.length > 0 && <div className="mt-6"><SearchResults results={searchResults} /></div>}
            {!isSearching && !searchError && searchResults.length === 0 && searchQuery && <div className="mt-6"><p className="text-center text-slate-500">No solutions found in this session for your query.</p></div> }
        </div>
      )}
    </div>
  );
